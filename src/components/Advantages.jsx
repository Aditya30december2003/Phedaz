  ...
  return ( 
    <section className="py-16">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-[75%] h-full  mx-auto">
          {advantages.map((advantage, index) => (
            <div
              key={index}
              className="rounded-lg overflow-hidden transition-all duration-300 shadow-xl border-blue-600 border-[0.1rem]"
            ...
            >
              <div className="p-6 flex flex-col items-center cursor-pointer hover:scale-105 duration-300" >
                ...
                <h2 className="text-xl font-semibold text-gray-800 mb-4 text-center">{advantage.Advantage}</h2>
                <button
                ...
                <button className={`${
                  ...
                  ? "bg-blue-700" 
                  : "bg-pink-700"
              } text-white animate px-6 py-6 mt-5 mr-[70%] rounded-full text-sm font-medium hover:${
                index%7 === 0
             ? "bg-purple-900"
             :  index%7 === 1
             ? "bg-yellow-700"
             ...
             ? "bg-blue-700" 
             : "bg-pink-700"
         } focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 outline-none transition-all duration-500 mix-blend-hard-light ${
                  isHovered ? "scale-[10.5]" : ""}`}
                  onClick={() => handleTabClick(index)}
                  ></button>
              </div>
              {selectedTab === index && (
      ...
      <div className="h-[40%] mb-5">
        <img src={advantage.image} alt="Advantage" className="w-[100%] rounded-xl shadow-md h-[100%] object-cover transition-transform duration-300 ease-in-out transform hover:scale-105" />
      </div>

      {/* Content Section */}
      <div>
        <h3 className="text-3xl font-semibold text-blue-900 mb-4 text-center tracking-wide">{advantage.Advantage}</h3>
        <p className="text-blue-700 mb-6 text-md leading-relaxed text-center mt-5">{advantage.Content}</p>
      </div>
    </div>
  </div>
)}
